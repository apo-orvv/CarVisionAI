...
mtx = save_dict['mtx']
dist = save_dict['dist']
window_size = 5  # frames for line smoothing
left_line = Line(n=window_size)
right_line = Line(n=window_size)
detected = False  
left_curve, right_curve = 0., 0.  # radius of curvature for left and right lanes
left_lane_inds, right_lane_inds = None, None  # calculating curvature


# MoviePy video annotation -> function
def annotate_image(img_in):
	...
	if not detected:
		...
		left_curve, right_curve = calc_curve(left_lane_inds, right_lane_inds, nonzerox, nonzeroy) # curvature
		...
		# update if we detected lines in current frame
		if ret is not None:
			left_fit = ret['left_fit']
			right_fit = ret['right_fit']
			nonzerox = ret['nonzerox']
	...
	# final visualization on top of original undistorted image
...
if __name__ == '__main__':
	annotate_video('testvid.mp4', 'op.mp4')
